...
class run {
    ...
    fun init() {
        ...
        req.open("GET", "http://localhost:8080/api/workload/" + workloadID, true)
        req.send()
    ...
    private fun sendWorkerInfoToBackend() {
        ...
        req.open("POST", "http://localhost:8080/api/run/worker", true)
    ...
    private fun checkStatusOfBackendWorker() {
        ...
        req.open("GET", "http://localhost:8080/api/run/ensureWorkerWaiting/" + workersetID, true)
        req.send()
    }
    ...
    private fun initWorkers() {
        ...
        req.open("GET", "http://localhost:8080/api/run/initWorker/" + workersetID + "?endpoint=" + tbxEndpoint.value, true)
        req.send()
    }
    ...
    fun refreshWorkerStatus() {
        ...
        for (w in worker.asList()) {
            ...
            var url = "http://localhost:8080/api/run/workerstatus?url=" + w.url
            val req = XMLHttpRequest()
            req.onloadend = fun(event: Event) {
